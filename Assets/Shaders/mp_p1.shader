Shader "Custom/Confmets/mp_p1"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            static const float2  k2  =  dual_lattice_vector( 0, 1 );
            static const float2  k3  =  dual_lattice_vector( 1, 1 );

            float mu( float2 p )
            {
                return  ( 6 + cop(p,k1) + cop(p,k2) + sip(p,k3) ) / 6;
            }

            float2 mu_grad( float2 p )
            {
                return  float2( cop_dx(p,k1) + cop_dx(p,k2) + sip_dx(p,k3),
                                cop_dy(p,k1) + cop_dy(p,k2) + sip_dy(p,k3)  ) / 6;
            }
