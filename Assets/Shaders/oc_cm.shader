Shader "Custom/Confmets/oc_cm"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            static const float a2 =   PI/u2p.w;

            float mu( float2 p )
            {
                return  ( 5 + cos(p.x*2*a1) + sin(p.x*2*a1) + 2*cos(p.x*a1)*cos(p.y*a2) ) / 5;
            }

            float2 mu_grad( float2 p )
            {
                return  float2( 2*a1*( -sin(p.x*2*a1) + cos(p.x*2*a1) - sin(p.x*a1)*cos(p.y*a2) ),
                                -2*a2*cos(p.x*a1)*sin(p.y*a2)                                      ) / 5;
            }
