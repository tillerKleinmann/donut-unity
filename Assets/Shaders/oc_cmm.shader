Shader "Custom/Confmets/oc_cmm"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float mu( float2 p )
            {
                return  ( 3 + cos(p.x*2*a1) + cos(p.x*a1)*cos(p.y*a2) ) / 3;
            }

            float2 mu_grad( float2 p )
            {
                return  float2( -a1*( 2*sin(p.x*2*a1) + sin(p.x*a1)*cos(p.y*a2) ),
                                -a2*cos(p.x*a1)*sin(p.y*a2)                        ) / 3;
            }
