Shader "Custom/Confmets/op_pg"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            static const float a2 = 2*PI/u2p.w;

            float mu( float2 p )
            {
                return  ( 6 + ( cos(p.x*2*a1) - sin(p.x*2*a1) )*cos(p.y*a2) + 2*cos(p.x*a1)*sin(p.y*a2) ) / 6;
            }

            float2 mu_grad( float2 p )
            {
                return  float2( -2*a1*( ( sin(p.x*2*a1) + cos(p.x*2*a1) )*cos(p.y*a2) +   sin(p.x*a1)*sin(p.y*a2) ),
                                  -a2*( ( cos(p.x*2*a1) - sin(p.x*2*a1) )*sin(p.y*a2) - 2*cos(p.x*a1)*cos(p.y*a2) )  ) / 6;
            }
