Shader "Custom/Confmets/op_pgg"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float mu( float2 p )
            {
                return  ( 4 + 2*cos(p.x*a1)*cos(p.y*a2) - sin(p.x*a1)*sin(p.y*2*a2) ) / 4;
            }

            float2 mu_grad( float2 p )
            {
                return  float2(   -a1*( 2*sin(p.x*a1)*cos(p.y*a2) + cos(p.x*a1)*sin(p.y*2*a2) ),
                                -2*a2*(   cos(p.x*a1)*sin(p.y*a2) + sin(p.x*a1)*cos(p.y*2*a2) )  ) / 4;
            }
