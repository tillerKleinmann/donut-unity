Shader "Custom/Confmets/sq_p4"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float mu( float2 p )
            {
                return ( 6 + cos(p.x) + cos(p.y) + cos(p.x+p.y/2) + cos(-p.x/2+p.y) ) / 6;
            }

            float2 mu_grad( float2 p )
            {
                return float2( -sin(p.x) - sin(p.x + p.y/2)   + sin(-p.x/2 + p.y)/2,
                               -sin(p.y) - sin(p.x + p.y/2)/2 - sin(-p.x/2 + p.y)    ) / 6;
            }
